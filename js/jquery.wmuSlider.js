  ...
  $.fn.wmuSlider = function (options) {
    ...
    return this.each(function () {
          ...
          if (!infinite) {
            ...
            if (index == 0) {
              ...
              if (!touch) {
                wrapper.css(
                  "marginLeft",
                  (-$this.width() / options.items) * slidesCount
                );
              }
              wrapper.animate(
                { marginLeft: (-$this.width() / options.items) * index },
      ...
      if (options.navigationControl) {
        var prev = $(
          '<a class="wmuSliderPrev">' + options.previousText + "</a>"
        );
        ...
        if (phase == "move" && (direction == "left" || direction == "right")) {
          if (direction == "right") {
            if (currentIndex == 0) {
              wrapper.css(
                "marginLeft",
                (-slidesCount * $this.width()) / options.items + distance
              );
            } else {
              wrapper.css(
                "marginLeft",
                (-currentIndex * $this.width()) / options.items + distance
              );
            }
